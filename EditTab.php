<h1 class="drw-main-heading">Editing <span class="dummy_text">Campaign:</span> <span class="camapaign_name">demo</span> <span class="mixed_value" style="display: none;"></span></h1>
<div class="edit-camp-form-outr">
	<!--campaigns edit-tab content -->
	<?php include 'CampaignEditTab.php';?>
	<!--end campaigns edit-tab content -->
	<!--Adsets edit-tab content -->
	<?php include 'AdsetsEditTab.php';?>
	<!--end Adsets edit-tab content -->
	<!--Ads edit-tab content -->
	<?php include 'AdsEditTab.php';?>
	<!--end Ads edit-tab content -->
	<?php //include 'AllPops.php';?>
</div>
<div class="save-editing-fix-bar">	
	<!-- <span>1 item to review</span>  -->    
	<button class="light-grey-btn close_edit_tab" rel="">Close</button>
	<button class="blue-btn confirm_close" rel="">Confirm and Close</button>
</div>