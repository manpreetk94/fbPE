<?php
	...
	if(!empty($result['data'])){
		foreach ($result['data'] as $key => $value) {
				...
				$spline1[$key]['label']=$value['gender'];
				if(isset($value[$res1])){
					$spline1[$key]['y']=$value[$res1];
				}else{
					$spline1[$key]['y']=0;
				}
				$spline2[$key]['label']=$value['gender'];
				if(isset($value[$res2])){
					$spline2[$key]['y']=$value[$res2];
				}else{
					$spline2[$key]['y']=0;
				}
				$spline3[$key]['label']=$value['gender'];
				if(isset($value[$res2])){
					$spline3[$key]['y']=$value[$res2];
				}else{
					$spline3[$key]['y']=0;
				}
...
<script type="text/javascript">
	...
	var chart = new CanvasJS.Chart("chartContainerAccountGender", {
...
<?php if(!empty($result['data'])){ ?>
		...
		<div id="chartContainerAccountGender" style="height: 300px; width: 100%;"></div> 
