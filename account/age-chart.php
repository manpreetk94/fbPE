<?php
	...
	if(!empty($result['data'])){
		foreach ($result['data'] as $key => $value) {
				...
				$spline1[$key]['label']=$value['age'];
				if(isset($value[$res1])){
					$spline1[$key]['y']=$value[$res1];
				}else{
					$spline1[$key]['y']=0;
				}
				$spline2[$key]['label']=$value['age'];
				if(isset($value[$res2])){
...
<script type="text/javascript">
	...
	var chart = new CanvasJS.Chart("chartContainerAccountAge", {
...
<?php if(!empty($result['data'])){ ?>
		...
		<div id="chartContainerAccountAge" style="height: 300px; width: 100%;"></div> 
